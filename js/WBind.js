...
//			this.prop[name] = e.value ;
			var v = _getprop(name) ;
			if(this._func[name].get) v = this._func[name].get(v,name) ;
			return v  ;
		},
		set:function(val) {
//			WBind.log("set "+name) 
			if(this._func[name].set) val = this._func[name].set(val,name) ;
			this.prop[name] = val ;
			_setval(name,val) ;
			if(this._func[name].change) this._func[name].change(_getprop(name),name) ;
			if(this._func[name].input) this._func[name].input(_getprop(name),name) ;
		}
	})
	var self = this ;
	var v = null ;
	...
	if((e instanceof NodeList || Array.isArray(e))) {
		...
		for(var i=0;i<e.length;i++) {
			...
			if(!exist) e[i].addEventListener("change", function(ev) {
				...
				self.prop[name] = val ;
				if(self._func[name].get) val = self._func[name].get(val,name) ;
//				WBind.log("get "+name+"="+val)
				if(self._func[name].change) self._func[name].change(val,name) ;
			})
...
//			WBind.log("get "+name+"="+val)
	...
	//		WBind.log("get "+name+"="+this.value)
			if(self._func[name].input) self._func[name].input(val,name) ;
		})
	}
	if(this._func[name].get) v = this._func[name].get(v,name) ;
	this.prop[name] = v ;
	if(self._func[name].input) this._func[name].input(v,name) ;
	if(self._func[name].change) this._func[name].change(v,name) ;
...
WBind.obj.prototype.bindAllInput = function(base,func) {
